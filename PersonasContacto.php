            ...
            <div class="card-header bg-white py-3">
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-center">
                    <h4 class="card-title mb-2 mb-md-0">
                        <i class="fas fa-list mr-2"></i> Personas Registradas
                    </h4>
                    <!-- Buscador -->
                    ...
                    <table class="table table-hover mb-0">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th class="border-0">Cliente Asociado</th>
        ...
        <div class="modal-content">
            ...
            <form id="formCrearPersonaContacto">
                <div class="modal-body p-4">
                     <div class="form-group"><label>Nombre *</label><input type="text" name="nombre_persona" class="form-control" required></div>
...
<div class="modal fade" id="modalEditarPersonaContacto" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg"><div class="modal-content">
        ...
        <form id="formEditarPersonaContacto">
            ...
            <div class="modal-body p-4">
                 <div class="form-group"><label>Nombre *</label><input type="text" id="editar_nombre_persona" name="nombre_persona" class="form-control" required></div>
                 <div class="form-group"><label>Cargo</label><input type="text" id="editar_cargo" name="cargo" class="form-control"></div>
                 <div class="form-group"><label>Teléfono</label><input type="text" id="editar_telefono" name="telefono" class="form-control"></div>
                 <div class="form-group"><label>Correo</label><input type="email" id="editar_correo" name="correo" class="form-control"></div>
                 <div class="form-group">
                     <label>Cliente Asociado *</label>
...
$(document).ready(function() {
    ...
    $('#buscador-personas').on('keypress', function(e) {
        if (e.which === 13) {
            cargarPersonas(1);
        }
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    Swal.fire('Error', response.message || 'No se pudo crear la persona.', 'error');
                }
            }
        });
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    cargarPersonas($('#paginacion-controles-personas .active .page-link').text() || 1);
                } else {
                    Swal.fire('Error', response.message || 'No se pudo actualizar la persona.', 'error');
                }
            }
        });
    });
...
function cargarPersonas(pagina) {
    const busqueda = $('#buscador-personas').val();
    ...
    $.ajax({
        ...
        dataType: 'json',
        beforeSend: function() {
            $('#tabla-personas-body').html('<tr><td colspan="7" class="text-center"><div class="spinner-border text-primary" role="status"><span class="sr-only">Cargando...</span></div></td></tr>');
        },
        success: function(response) {
            if (response.success) {
                ...
                if (personas.length > 0) {
                    ...
                    $('#tabla-personas-body').append('<tr><td colspan="7" class="text-center empty-state"><i class="fas fa-search fa-2x text-muted mb-3"></i><p>No se encontraron personas para la búsqueda.</p></td></tr>');
                }
...
function actualizarPaginacionPersonas(paginacion) {
    ...
    $('#paginacion-controles-personas').empty();
    $('#info-paginacion-personas').empty();
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                const detallesHtml = `
                    <p><strong>ID:</strong> ${p.id_persona}</p>
                    <p><strong>Nombre:</strong> ${p.nombre_persona}</p>
                    <p><strong>Cargo:</strong> ${p.cargo || '-'}</p>
                    <p><strong>Teléfono:</strong> ${p.telefono || '-'}</p>
                    <p><strong>Correo:</strong> ${p.correo || '-'}</p>
                    <p><strong>Cliente:</strong> ${p.nombre_cliente || 'N/A'}</p>
                `;
                $('#detalles-persona-body').html(detallesHtml);
